...
export const CheckoutPage = () => {
    ...
    try {
      ...
      const pedidoPayload: CrearPedidoRequest = {
        items: cartItems.map(item => ({
          ...
          personalizacion:
            item.exclusiones.length > 0 ? item.exclusiones.map(Number) : null,
        })),
        forma_pago_codigo,
    ...
    return (
      <div className="min-h-screen flex flex-col bg-cream-50 font-sans">
        ...
        <main className="flex-1 flex items-center justify-center px-4 sm:px-6 py-12 relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-md w-full bg-paper-0 rounded-2xl border border-paper-200 shadow-xl p-8 text-center space-y-6 animate-scaleUp">
            <div className="w-24 h-24 bg-success-50 ring-4 ring-success-100 rounded-2xl flex items-center justify-center text-success-600 mx-auto">
            ...
            <div className="bg-cream-100 border border-cream-200 p-4 rounded-xl text-left space-y-2">
              ...
              <div className="flex justify-between text-sm text-ink-700 font-semibold border-t border-cream-300 pt-2 mt-2">
            ...
            <div className="grid grid-cols-1 gap-3 pt-2">
              <button
                ...
                className="w-full bg-brand-yellow-400 hover:bg-brand-yellow-500 active:bg-brand-yellow-600 text-ink-900 font-bold py-3.5 rounded-xl shadow-yellow hover:shadow-lg transition-all duration-150 active:scale-[0.98] cursor-pointer"
              >
                Ver mis pedidos
              </button>
              <button
                onClick={() => navigate('/')}
                className="w-full bg-paper-0 border-2 border-paper-200 hover:border-ink-700 hover:bg-paper-50 text-ink-900 font-semibold py-3 rounded-xl transition-all duration-150 cursor-pointer"
              >
                Volver al Menú
              </button>
            </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-cream-50 font-sans">
      ...
      {/* Stepper */}
      <div className="w-full bg-paper-0 border-b border-paper-200 py-4 px-4 sm:px-6 shrink-0">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <section className="lg:col-span-2 space-y-6 min-w-0">
          {globalError && (
            <div className="bg-danger-50 border border-danger-200 text-danger-700 text-sm px-4 py-3 rounded-xl flex items-start gap-2.5 font-medium leading-relaxed animate-shake">
          ...
          <div className={`${cardBase} p-6 space-y-5`}>
            <header className="flex items-center gap-2">
              <span className="w-8 h-8 rounded-xl bg-brand-red-50 text-brand-red-600 flex items-center justify-center shrink-0">
                <Truck size={16} />
              </span>
              <h3 className={`${eyebrow} flex items-center gap-1.5`}>
                <span>01 · Método de Entrega</span>
              </h3>
            </header>
            ...
            {tipoEntrega === 'DELIVERY' && (
              <div className="pt-5 border-t border-paper-200 space-y-3">
                ...
                {direcciones.length === 0 ? (
                  <div className="text-center py-8 border border-dashed border-paper-300 rounded-xl p-4 bg-cream-50 space-y-3">
                    ...
                    <button
                      ...
                      className="bg-paper-0 border-2 border-paper-200 hover:border-ink-700 hover:bg-paper-50 text-ink-900 font-semibold px-4 py-2 rounded-xl transition-all duration-150 cursor-pointer text-sm"
                    >
                      Registrar Dirección
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {direcciones.map(dir => (
                      <div
                        ...
                        className={`p-4 border-2 rounded-xl cursor-pointer select-none transition-all ${
                          selectedDireccionId === dir.id
                            ? 'bg-brand-red-50 border-brand-red-500 text-brand-red-900 shadow-sm'
                            : 'bg-paper-0 border-paper-200 hover:bg-cream-50 hover:border-brand-yellow-300 text-ink-700'
                        }`}
                      >
            ...
            {tipoEntrega === 'TAKE_AWAY' && (
              <div className="bg-cream-100 border border-brand-yellow-200 p-4 rounded-xl flex gap-3 text-left mt-4 animate-fadeIn">
                ...
                <div>
                  ...
                  <p className="text-xs text-ink-700 leading-relaxed mt-1">
                    ...
                    ¡Te avisaremos por el rastreador!
                  </p>
                </div>
              </div>
            )}
          ...
          <div className={`${cardBase} p-6 space-y-5`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                ...
                <button
                  ...
                  className={`py-3 px-3 border-2 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all cursor-pointer ${
                    metodoPago === p.id
                      ? 'bg-brand-red-50 border-brand-red-500 text-brand-red-700'
                      : 'bg-paper-0 border-paper-200 text-ink-600 hover:bg-cream-50 hover:border-brand-yellow-300'
                  }`}
                >
                  {p.icon}
                  <span>{p.name}</span>
            ...
            {metodoPago === 'MERCADOPAGO' && (
              <div className="pt-5 border-t border-paper-200 space-y-6 animate-slideDown">
                ...
                <div className="w-full max-w-sm mx-auto aspect-video rounded-2xl p-5 text-paper-0 flex flex-col justify-between shadow-xl relative overflow-hidden select-none bg-gradient-to-br from-ink-900 via-ink-800 to-wine-800">
                  ...
                  <div className="flex items-end justify-between font-mono text-xs tracking-wider uppercase text-paper-300 relative">
                    <div className="truncate pr-4 max-w-[200px]">
                      ...
                      <span className="font-bold text-paper-0 truncate">
                        {cardName || 'JUAN PEREZ'}
                      </span>
                    </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 text-xs text-success-700 font-bold border-b border-paper-200 pb-2">
                    <Lock size={14} />
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className={`${inputBase} text-center tracking-widest font-bold`}
                      />
                    </div>
                    <div>
            ...
            {metodoPago === 'EFECTIVO' && (
              <div className="bg-cream-100 border border-brand-yellow-200 p-4 rounded-xl text-sm text-ink-900 leading-relaxed font-medium animate-slideDown mt-4 flex gap-3">
            ...
            {metodoPago === 'TRANSFERENCIA' && (
              <div className="bg-info-50 border border-info-200 p-4 rounded-xl text-sm text-ink-900 space-y-2 leading-relaxed font-medium animate-slideDown mt-4">
                ...
                <p className="text-[11px] text-info-700 italic pt-2 border-t border-info-200">
        ...
        <aside className={`${cardBase} p-6 space-y-6 sticky top-24`}>
          <header className="flex items-center gap-2 pb-3 border-b border-paper-200">
            <span className="w-8 h-8 rounded-xl bg-brand-red-50 text-brand-red-600 flex items-center justify-center shrink-0">
              <ShoppingBag size={16} />
            </span>
          ...
          <div className="border-t border-paper-200 pt-4 space-y-2">
            ...
            />
            <div className="flex justify-between items-end border-t border-paper-200 pt-3">
          ...
          <button
            ...
            className="w-full bg-brand-red-500 hover:bg-brand-red-600 active:bg-brand-red-700 text-white font-bold py-4 rounded-xl shadow-brand hover:shadow-lg transition-all duration-150 active:scale-[0.98] disabled:bg-ink-200 disabled:text-ink-400 disabled:cursor-not-allowed disabled:shadow-none cursor-pointer flex items-center justify-center gap-2 text-base"
          >
            <Lock size={18} />
            <span>
              {estadoLocal === 'cerrado' ? 'Local Cerrado temporalmente' : 'Confirmar y Comprar'}
            </span>
          </button>
      ...
      {/* Modal procesando */}
      {isProcessing && (
        <div className="fixed inset-0 z-[60] flex flex-col items-center justify-center p-4 bg-ink-900/55 backdrop-blur-sm animate-fadeIn">
      ...
      {/* Modal agregar dirección rápida */}
      {isAddrModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fadeIn">
          ...
          >
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              {addrError && (
                <div className="bg-danger-50 border border-danger-200 text-danger-700 text-sm px-4 py-3 rounded-xl flex items-center gap-2 font-medium animate-shake">
              ...
              <div>
                ...
                <textarea
                  placeholder="Ej: Tocar timbre 'B'…"
                  rows={2}
                  value={newIndicaciones}
                  onChange={e => setNewIndicaciones(e.target.value)}
...
const DeliveryButton = ({
  ...
  <button
    onClick={onClick}
    className={`py-4 px-4 border-2 rounded-xl font-bold text-sm flex flex-col items-center gap-2 transition-all cursor-pointer ${
      active
        ? 'bg-brand-red-50 border-brand-red-500 text-brand-red-700'
        : 'bg-paper-0 border-paper-200 text-ink-600 hover:bg-cream-50 hover:border-brand-yellow-300'
    }`}
...
const Step = ({
  ...
  return (
    <div className="flex items-center gap-2 shrink-0">
      ...
      <span className={`text-xs font-black uppercase tracking-wider hidden sm:block ${text}`}>
        {label}
      </span>
    </div>
  );
...
const StepDivider = ({ done }: { done?: boolean }) => (
  <div className={`h-0.5 w-8 sm:w-16 rounded-full ${done ? 'bg-brand-red-500' : 'bg-paper-200'}`} />
);
