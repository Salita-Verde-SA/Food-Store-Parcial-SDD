    ...
    <div className="min-h-screen flex flex-col bg-cream-50 font-sans">
      ...
      <section className="relative overflow-hidden bg-brand-red-500 text-white">
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-16 md:py-24 text-center space-y-6">
          <div className="inline-flex items-center gap-2 bg-brand-yellow-400 text-ink-900 px-3.5 py-1.5 rounded-full text-[11px] font-black uppercase tracking-widest shadow-sm">
            <Sparkles size={13} />
      ...
      {estadoLocal === 'cerrado' && (
        <div className="bg-danger-50 border-b-2 border-danger-200 text-danger-700 px-4 sm:px-6 py-3.5 flex items-center justify-center gap-3 animate-fadeIn shrink-0">
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-8 flex flex-col md:flex-row gap-6 items-start">
        {/* ───────── FILTROS LATERALES ───────── */}
        <aside className="w-full md:w-72 space-y-5 shrink-0 md:sticky md:top-24">
          {/* Buscador */}
                ...
                {alergenos.map(ing => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-2.5 px-3 py-2 rounded-lg cursor-pointer select-none transition-colors duration-150 ${
                        isChecked
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
                {catalogData.items.map((prod: Producto) => {
                  ...
                  return (
                    <article
                      key={prod.id}
                      className={`${cardBase} hover:shadow-card-hover hover:border-brand-yellow-300 transition-all duration-200 flex flex-col overflow-hidden relative group`}
                    >
                      {/* Imagen */}
                      <div className="aspect-[5/4] bg-cream-100 relative flex items-center justify-center overflow-hidden">
                        {prod.imagen_url ? (
                          <img
                            src={prod.imagen_url}
                            alt={prod.nombre}
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-ink-900/30 via-transparent to-transparent pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

                        {containsAllergen && (
                          <div className="absolute top-3 right-3 inline-flex items-center gap-1 bg-paper-0/95 text-danger-700 border border-danger-200 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm backdrop-blur-sm">
                            <ShieldAlert size={11} />
                            <span>Alérgeno</span>
                          </div>
                        )}
                        ...
                        {isClosed && (
                          <div className="absolute inset-0 bg-ink-900/55 flex items-center justify-center backdrop-blur-[2px]">
                            <span className="text-white font-display font-black uppercase tracking-widest text-base px-3 py-1.5 border-2 border-white rounded-full">
                              Local Cerrado
                            </span>
                          </div>
                        )}
                      ...
                      <div className="p-5 flex-1 flex flex-col justify-between gap-4">
                              ...
                              {prod.ingredientes.slice(0, 4).map(ing => (
                                <span
                                  ...
                                  className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${
                                    ing.es_alergeno
                                      ? 'bg-danger-50 text-danger-700'
                                      : 'bg-cream-100 text-ink-700 border border-cream-200'
                                  }`}
                                >
                                  {ing.nombre}
                                </span>
                        ...
                        <div className="flex items-center justify-between gap-3 pt-3 border-t border-paper-200">
                          <div>
                          ...
                          <button
                            ...
                            disabled={isClosed}
                            className="bg-brand-red-500 hover:bg-brand-red-600 active:bg-brand-red-700 text-white font-bold px-4 py-2.5 rounded-xl shadow-sm hover:shadow-brand transition-all duration-150 active:scale-[0.96] disabled:bg-ink-200 disabled:text-ink-400 disabled:cursor-not-allowed disabled:shadow-none cursor-pointer text-sm flex items-center gap-1.5"
                          >
                            <Plus size={15} />
                            <span>Agregar</span>
                          </button>
              ...
              {catalogData.total > limit && (
                <div className="flex items-center justify-center gap-3 pt-8 border-t border-paper-200">
                  <button
                    disabled={page === 0}
                    onClick={() => setPage(p => p - 1)}
                    className="bg-paper-0 border-2 border-paper-200 hover:border-ink-700 hover:bg-paper-50 text-ink-900 font-semibold px-4 py-2 rounded-xl transition-all duration-150 cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed text-sm"
                  >
                    Anterior
                  </button>
      ...
      {customizingProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          />
          <div className="relative bg-paper-0 rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col animate-scaleUp border border-paper-200">
            {/* Banner gradiente */}
            ...
            <div className="px-6 py-5 border-b border-paper-200 flex items-center justify-between shrink-0">
              ...
              <button
                onClick={() => setCustomizingProduct(null)}
                className="w-10 h-10 flex items-center justify-center rounded-xl bg-paper-0 border border-paper-200 hover:bg-paper-100 text-ink-700 transition-colors duration-150 cursor-pointer shrink-0 ml-3"
                aria-label="Cerrar"
              >
                <X size={18} />
              </button>
            ...
            <div className="px-6 py-5 space-y-4 overflow-y-auto">
              <div className="bg-brand-yellow-50 border border-brand-yellow-200 p-4 rounded-xl flex gap-3">
                  ...
                  {customizingProduct.ingredientes.map(ing => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer select-none transition-colors duration-150 ${
                          isExcluded
                            ? 'bg-danger-50 border-danger-200 text-danger-700 line-through'
                            : 'bg-paper-0 border-paper-200 hover:border-brand-yellow-300 hover:bg-cream-50 text-ink-900'
                        }`}
                      >
                        ...
                        {ing.es_alergeno && (
                          <span className="inline-flex items-center gap-1 bg-danger-50 text-danger-700 border border-danger-200 text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full">
                            Alérgeno
                          </span>
                        )}
                      </div>
            ...
            <div className="px-6 py-4 border-t border-paper-200 bg-paper-50 flex items-center justify-end gap-3 shrink-0">
              ...
              <button
                ...
                className="bg-brand-red-500 hover:bg-brand-red-600 active:bg-brand-red-700 text-white font-bold px-5 py-2.5 rounded-xl shadow-brand hover:shadow-lg transition-all duration-150 active:scale-[0.98] cursor-pointer text-sm"
              >
                Confirmar y agregar
              </button>
            </div>
      ...
      {isCartOpen && (
        <>
          <div
            ...
            className="fixed inset-0 bg-ink-900/55 backdrop-blur-sm z-40 animate-fadeIn"
          />

          <aside className="fixed inset-y-0 right-0 w-full sm:w-[440px] bg-paper-0 shadow-xl z-50 flex flex-col animate-slideInRight">
            <div className="bg-brand-red-500 text-white px-6 py-5 flex items-center justify-between shrink-0 relative overflow-hidden">
              ...
              <button
                onClick={() => setIsCartOpen(false)}
                className="w-10 h-10 flex items-center justify-center rounded-xl bg-white/15 hover:bg-white/25 text-white transition-colors duration-150 cursor-pointer relative z-10"
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-3 bg-cream-50">
              {cartItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center gap-4">
                  <div className="w-20 h-20 rounded-2xl bg-cream-100 flex items-center justify-center">
                  ...
                  <button
                    onClick={() => setIsCartOpen(false)}
                    className="bg-brand-yellow-400 hover:bg-brand-yellow-500 active:bg-brand-yellow-600 text-ink-900 font-bold px-5 py-2.5 rounded-xl shadow-yellow hover:shadow-lg transition-all duration-150 active:scale-[0.98] cursor-pointer text-sm mt-2"
                  >
                    Ver el menú
                  </button>
                </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center rounded-lg bg-paper-0 border border-paper-200 hover:bg-paper-100 hover:border-paper-300 text-ink-700 cursor-pointer transition-colors duration-150 disabled:opacity-40 disabled:cursor-not-allowed"
                          aria-label="Restar"
                        >
                          <Minus size={12} />
                        </button>
                        ...
                        <button
                          onClick={() => updateQuantity(item.cart_item_key, item.cantidad + 1)}
                          className="w-8 h-8 flex items-center justify-center rounded-lg bg-paper-0 border border-paper-200 hover:bg-paper-100 hover:border-paper-300 text-ink-700 cursor-pointer transition-colors duration-150"
                          aria-label="Sumar"
                        >
                          <Plus size={12} />
                        </button>
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center rounded-lg bg-paper-0 border border-danger-100 hover:bg-danger-50 text-danger-600 cursor-pointer transition-colors duration-150 ml-1"
                    ...
                    {item.exclusiones_nombres && item.exclusiones_nombres.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {item.exclusiones_nombres.map((name, i) => (
                          <span
                            key={i}
                            className="inline-flex items-center gap-1 text-[10px] font-bold text-ink-700 bg-paper-100 border border-paper-200 px-2 py-0.5 rounded-full"
                          >
                            Sin {name}
                          </span>
                        ))}
            ...
            {cartItems.length > 0 && (
              <div className="border-t border-paper-200 bg-paper-0 p-6 space-y-4 shrink-0">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <button
                    onClick={clearCart}
                    className="col-span-1 bg-paper-0 border border-danger-100 hover:bg-danger-50 hover:border-danger-200 text-danger-600 hover:text-danger-700 font-semibold px-3 py-3 rounded-xl transition-colors duration-150 cursor-pointer text-sm flex items-center justify-center gap-1.5"
                  >
                    <Trash2 size={14} />
                  ...
                  <button
                    ...
                    className="col-span-2 bg-brand-red-500 hover:bg-brand-red-600 active:bg-brand-red-700 text-white font-bold py-3 rounded-xl shadow-brand hover:shadow-lg transition-all duration-150 active:scale-[0.98] disabled:bg-ink-200 disabled:text-ink-400 disabled:cursor-not-allowed disabled:shadow-none cursor-pointer text-sm"
                  >
                    Confirmar pedido
                  </button>
                </div>
...
const HeroBadge = ({ icon, label }: { icon: React.ReactNode; label: string }) => (
  <span className="inline-flex items-center gap-1.5 bg-paper-0/10 backdrop-blur-sm border border-paper-0/20 text-white px-3 py-1.5 rounded-full">
...
const CategoryButton = ({ active, label, onClick, icon }: CategoryButtonProps) => (
  <button
    onClick={onClick}
    className={`w-full text-left px-3 py-2.5 rounded-lg text-sm font-semibold transition-colors duration-150 flex items-center justify-between cursor-pointer ${
      active
